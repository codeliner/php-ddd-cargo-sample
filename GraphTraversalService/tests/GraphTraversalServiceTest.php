...
use Codeliner\GraphTraversalService\GraphTraversalService;
...
class GraphTraversalServiceTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function it_finds_a_list_of_transit_paths()
    {
        ...
        $this->assertInstanceOf('Codeliner\GraphTraversalService\Dto\TransitPathDto', $transitPath);

        $edges = $transitPath->getEdges();

        $this->assertEquals(3, count($edges));

        $this->assertInstanceOf('Codeliner\GraphTraversalService\Dto\EdgeDto', $edges[0]);
